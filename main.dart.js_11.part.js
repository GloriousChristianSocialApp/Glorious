((a,b)=>{a[b]=a[b]||{}})(self,"$__dart_deferred_initializers__")
$__dart_deferred_initializers__.current=function(a,b,c,$){var J,B,C,A={
amq(d,e,f){var x,w=null
if(f==null)x=e!=null?new B.bA(e,w,w,w,w,w,C.F):w
else x=f
return new A.uJ(d,x,w)},
uJ:function uJ(d,e,f){this.c=d
this.e=e
this.a=f},
Mj:function Mj(d){var _=this
_.d=d
_.c=_.a=_.e=null},
G_:function G_(d,e,f,g){var _=this
_.f=_.e=null
_.r=!0
_.w=d
_.a=e
_.b=f
_.c=g},
b4S(d){var x,w,v,u={}
u.a=d
x=y.b
w=d.hR(x)
v=!0
while(!0){if(!(v&&w!=null))break
v=x.a(d.yZ(w)).f
w.ly(new A.aBH(u))
w=u.a.hR(x)}return v},
aBH:function aBH(d){this.a=d},
c9:function c9(){}}
J=c[1]
B=c[0]
C=c[2]
A=a.updateHolder(c[49],A)
A.uJ.prototype={
gau1(){var x,w,v,u=this.e,t=u==null?null:u.gc1(u)
$label0$0:{x=t==null
w=x
if(w){u=C.ae
break $label0$0}w=t instanceof B.dg
if(w){v=t==null?y.m.a(t):t
u=v
break $label0$0}null.toString
u=null.E(0,u.gc1(u))
break $label0$0}return u},
a5(){return new A.Mj(new B.b9(null,y.j))}}
A.Mj.prototype={
apv(){this.e=null},
ep(){var x=this.e
if(x!=null)x.l()
this.of()},
aiC(d){var x,w,v,u=this,t=null,s=u.e,r=u.a
if(s==null){s=r.e
r=A.b4S(d)
x=B.D_(d,t)
w=B.anI(d,y.o)
w.toString
v=$.am.ak$.x.h(0,u.d).ga2()
v.toString
v=new A.G_(x,w,y.g.a(v),u.gapu())
v.saI(s)
v.sa6I(r)
w.ED(v)
u.e=v}else{s.saI(r.e)
s=u.e
s.toString
s.sa6I(A.b4S(d))
s=u.e
s.toString
s.soR(B.D_(d,t))}s=u.a.c
return s==null?new B.e1(C.j0,t,t):s},
I(d){var x=this,w=x.a.gau1()
x.a.toString
return new B.aX(w,new B.f5(x.gaiB(),null),x.d)}}
A.G_.prototype={
saI(d){var x,w=this
if(J.f(d,w.f))return
w.f=d
x=w.e
if(x!=null)x.l()
x=w.f
w.e=x==null?null:x.uS(w.ganJ())
w.a.aM()},
sa6I(d){if(d===this.r)return
this.r=d
this.a.aM()},
soR(d){if(d.k(0,this.w))return
this.w=d
this.a.aM()},
anK(){this.a.aM()},
l(){var x=this.e
if(x!=null)x.l()
this.mK()},
Hm(d,e){var x,w,v,u=this
if(u.e==null||!u.r)return
x=B.aqx(e)
w=u.w.OK(u.b.gt(0))
if(x==null){v=d.a.a
J.aE(v.save())
d.af(0,e.a)
u.e.iz(d,C.j,w)
v.restore()}else u.e.iz(d,x,w)}}
A.c9.prototype={}
var z=a.updateTypes(["~()","h(P)"])
A.aBH.prototype={
$1(d){this.a.a=d
return!1},
$S:32};(function installTearOffs(){var x=a._instance_0u,w=a._instance_1u
var v
x(v=A.Mj.prototype,"gapu","apv",0)
w(v,"gaiB","aiC",1)
x(A.G_.prototype,"ganJ","anK",0)})();(function inheritance(){var x=a.inherit
x(A.uJ,B.W)
x(A.Mj,B.X)
x(A.G_,B.li)
x(A.aBH,B.ct)
x(A.c9,B.H)})()
B.bO(b.typeUniverse,JSON.parse('{"uJ":{"W":[],"h":[]},"Mj":{"X":["uJ"]},"G_":{"li":[]}}'))
var y={m:B.L("dg"),j:B.L("b9<X<W>>"),g:B.L("C"),o:B.L("x1"),b:B.L("Ph")}};
(a=>{a["Ap1NVQOsegTW+hC+HBmHmerWLjo="]=a.current})($__dart_deferred_initializers__);